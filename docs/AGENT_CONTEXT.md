...
5. UI-задача не считается завершённой, если она сделана только для одного из
   языков `ru/en`.
6. Стартовая локация и стартовые организации не должны быть захардкожены по
   setting/genre. `Начальная точка` / `Starting Point` — только технический
   placeholder; реальную локацию должна вернуть модель в `state_changes.location`.
7. Butterfly simulation не должна сидить шаблонные гильдии, рынки, стражу,
   караваны или погоду до AI-сцены. Новые сущности появляются из текущей
   AI-локации и model-provided `impact_seeds`.
...
- не добавлять setting-based словари стартовых локаций или generic world seeds,
  которые будут повторяться между уникальными историями.
