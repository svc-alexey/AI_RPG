...
#### Старт из библиотеки миров

Если пользователь нажимает «Начать кампанию» на карточке истории из библиотеки,
экран новой кампании сначала фиксирует выбранную историю как seed и показывает
выбор формата:
...
│  ← Новая кампания           │
│                             │
│  Какой формат запустить?    │
│                             │
│  Выбранная история          │
│  <Название и краткое резюме> │
│                             │
│  ⚡ Короткая история         │
│  Быстрый старт: имя и пол   │
│                             │
│  🧭 Длинная кампания        │
│  Пошаговая настройка        │
└─────────────────────────────┘
...
Правила:
- выбранная история не меняется на этом шаге;
- `Короткая история` открывает quick start с уже подставленным prompt шаблона;
- `Длинная кампания` открывает detailed wizard сразу со step `Foundation`,
  потому что жанр и сеттинг берутся из шаблона;
- на шаге `Story` prompt шаблона уже находится в поле и доступен для правки;
- в library-flow выбранный `Story Mode` показывается как фиксированное значение,
  чтобы пользователь не переключил формат после выбора.
...
Быстрый старт теперь intentionally lightweight:
1. Имя героя (TextField)
2. Пол (ChoiceChips / селектор)

Остальное выбирается автоматически:
- сеттинг — случайно
- литературный жанр — случайно
- Story Mode — случайно, с bias `shortStory 70% / longCampaign 30%`

Если выпадает `longCampaign`, первый auto-intro в чате должен показывать
видимый пролог перед стартовой сценой.
...
- `Короткая история` — быстрый вход в действие, более компактные ответы ИИ
- `Длинная кампания` — отдельный видимый пролог на первом ходе, более
  развернутые ответы и более длинная story seed
...
Поведение генерации:
- для `Короткая история` промпт должен быть компактным и быстро выводить к
  завязке;
- для `Длинная кампания` промпт должен включать предысторию персонажа или
  мира, более длинную арку и richer narrative tone.
