...
4. После реализации: если для видимости изменений требуется перезапуск приложения, агент перезапускает его (например, `flutter run` или hot restart). Не оставлять это пользователю.
