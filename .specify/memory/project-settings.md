...
7. Если пользователь просит выполнить фичу целиком, агент обязан пройти весь этот pipeline автономно, не ожидая отдельного подтверждения между этапами, кроме случаев скрытого риска или неоднозначного выбора.
...
4. Для каждой новой фичи должна создаваться отдельная git-ветка в формате `codex/<feature-slug>`.
5. Внутри feature-папки обязаны существовать:
   - `01-Architecture.md`
   - `02-PRD.md`
   - `03-Implementation.md`
   - `04-QA.md`
6. Перед заполнением feature-документов нужно учитывать:
   - `PRD.md`
   - `Architecture.md`
   - `FlutterRules.md`
   - `ImplementationPlan.md`
   ...
   - `.specify/memory/project-context.md`
   - `.specify/memory/project-settings.md`
7. Реализация не должна стартовать, пока не заполнены архитектурный и аналитический этапы, кроме явного исключения для bugfix.
8. Допускается, что пока одна фича реализуется, другие фичи могут параллельно проходить архитектурный или аналитический этап.
9. Для новой фичи каждый ролевой этап должен вестись в отдельном окне или треде:
   - отдельное окно архитектора
   - отдельное окно аналитика
   - отдельное окно разработчика
   - отдельное окно тестировщика
10. Смешивать несколько ролей в одном окне допускается только для локального bugfix или явного исключения.
11. В автономном режиме агент сам последовательно проходит эти ролевые этапы, создает feature-ветку и обновляет документы и чекбоксы по ходу выполнения.
12. После завершения фичи агент не должен оставлять ее только в feature-ветке: если реализация рабочая и проверки пройдены, feature-ветка сливается в `master`.
13. После merge в `master` агент обязан повторно прогнать проверки на итоговом объединенном состоянии.
14. `master` хранит только стабильное состояние, прошедшее проверки.
